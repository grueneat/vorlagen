...
build:
  ...
  script: build.py
format: A4
id: 26-03-leporello-z-falz-99x210-6-seitig-portrait
idml_source: ../../originals/26-03-Leporello z-Falz 99x210 6-seitig Portrait Ordner/26-03-Leporello
  z-Falz 99x210 6-seitig Portrait.idml
previews_for_sla: fa6ab7df97be13ba25b7192b4b4e4b11076fef66c3a0492982eba8006f0b4446
title: 26-03 Leporello z-Falz 99×210 6-seitig (Portrait)
version: 0.1.0
build_py_sha256: 20bc1828028b982241c9897ff8ab82415342ef43ec9d19ffecad53e9d8a8c7ad
category: falzflyer
category_label: Falzflyer
variant_label: Z-Falz 6-seitig (Portrait)
brand_overrides:
- id: brand:line_spacing_0.9
  reason: IDML-imported template. The original InDesign ParagraphStyles (idml/headline-in-gruenem-kasten,
    ...
    Same gap class as the sibling falz templates.
- id: brand:font_family
  reason: 'IDML-imported template. 11 text frames inherit FONT="Times Roman" via the
    ...
    actual Run() calls inside these frames all use brand fonts (Gotham Narrow Book,
    Gotham Narrow Bold, Gotham Narrow Ultra, Vollkorn Black Italic); only the abstract
    paragraph-style fallback uses Times Roman. The converter cannot remap these to
    brand fonts without changing the baseline.pdf convergence target. Resolution path:
    extend tools/idml_to_dsl.py font resolution; tracked alongside issue 37 as a converter
    completeness gap. Same gap class as the sibling falz templates.'
- id: brand:bleed_3mm
...
- id: brand:inside_page
  reason: IDML-imported template. Multiple frames in the original IDML extend slightly
    outside the trim box (u1ae background polygon with 1.82mm overshoot; u3a2 and
    several decorative shapes with similar overshoot). These were intentional InDesign-side
...
- id: brand:image_text_overlap
  reason: IDML-imported template. The IDML places impressum text and other white-on-green
    ...
    "text on a raster image" (overlap concern). Same gap class as the sibling falz
    templates.
ci_overrides:
  non_ci_styles:
  - idml/absatzformat-1
...
- label: Vollständig (SLA + PDF)
  sla: /templates/26-03-leporello-z-falz-99x210-6-seitig-portrait/template.sla
  pdf: /templates/26-03-leporello-z-falz-99x210-6-seitig-portrait/preview.pdf
_previews:
- label: Seite 1
  src: /templates/26-03-leporello-z-falz-99x210-6-seitig-portrait/page-01.png
- label: Seite 2
  src: /templates/26-03-leporello-z-falz-99x210-6-seitig-portrait/page-02.png
---
