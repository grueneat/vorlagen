...
**Scope this run:** T01-T04 (both bugs fixed end-to-end) + T05 (experiments nav link, follow-up before ship)
...
- [x] T05 — experiments navigation link — `9192299` — added BASE_URL-aware `url()` helper to `site/src/layouts/Base.astro`, restyled the global header as a flex container (`space-between`, `flex-wrap` for mobile) with a `<nav>` containing two links: **Galerie** (`/vorlagen/`) and **Design-Experimente** (`/vorlagen/experiments/`). Hover state uses `--gruen-hell`. Additionally added a brand-aligned discovery card (`--gruen-dunkel` background, white text, "Ansehen →" badge) at the top of `site/src/pages/index.astro` so the experiments page is obvious on a first visit. Build emits 12 pages, exit 0; built HTML on `/`, `/templates/.../`, and `/experiments/` all contain the `href="/vorlagen/experiments/"` anchor.

## Verification status

- Python unit tests: **PASS — 866 tests** (`python3 -m unittest discover tools/sla_lib/tests`)
- `npm --prefix site run build`: **PASS** (12 pages built, 0 errors — re-verified after T05; `/vorlagen/experiments/` anchor present in built HTML for gallery index, template detail pages, and the experiments index itself)
...
- [x] All commits exist on branch: `742a3f3`, `63a0dc6`, `4de09ad`, `d37502f`, `9192299` (verified via `git log --oneline -6`)
