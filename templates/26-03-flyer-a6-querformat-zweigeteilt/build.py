"""26-03-flyer-a6-querformat-zweigeteilt — DSL build entry point.

Auto-generated from 26-03-Flyer A6 Querformat zweigeteilt.idml by tools/idml_to_dsl.py.
...
def build_template() -> Document:
    ...
    doc = Document(
        ...
        title='26-03-flyer-a6-querformat-zweigeteilt',
        template_id='26-03-flyer-a6-querformat-zweigeteilt',
...
def _add_page_0(doc: Document, page0) -> None:  # overrides task-3 stub
    ...
    # noinject: ziesel.jpg is a CMYK JPEG; Scribus 1.6.x renders CMYK JPEGs
    # near-blank (authoring-bug, accepted residual — see TOLERANCE_LOG.md).
    page0.add(ImageFrame(
        ...
        anname='uace',
        layer=0,
        image='../../shared/assets/26-03-flyer-a6-querformat-zweigeteilt/ziesel.jpg',
        local_scale=(0.447907, 0.447907),
        scale_type=1,
        local_offset_mm=(-16.8, 0),
    ))
    page0.add(Polygon(
        x_mm=74,
        y_mm=-3,
    ...
    page0.add(ImageFrame(
        ...
        anname='uad7',
        layer=0,
        # tune: uad7 logo invisible via inline_image_data (Scribus 1.6.x SCALETYPE=1
        # RGBA white-on-transparent bug); switch to image= ref + scale_type=0.
        image='../../shared/assets/26-03-flyer-a6-querformat-zweigeteilt/gruene-logo-bund-weiss-cmyk.png',
        scale_type=0,
    ))
    page0.add(Polygon(
        x_mm=60.898,
    ...
    # h_mm widened 27.6054mm→49.0361mm: Scribus clips lines when frame_h < 3 explicit lines × line height (leading=27.00pt; IDML overflows silently)
    page0.add(TextFrame(
        x_mm=78.5,
        y_mm=58.6807,
        w_mm=58.7292,
        ...
        layer=0,
        style='idml/normalparagraphstyle',
        runs=[Run(text='Das ist die ', font='Gotham Narrow Ultra', fontsize=30, fcolor='White', paragraph_style='idml/normalparagraphstyle', paragraph_attrs={'ALIGN': '0', 'LINESPMode': '1'}), Run(text='', has_itext=False, paragraph_style='idml/normalparagraphstyle', paragraph_attrs={'ALIGN': '0', 'LINESPMode': '1'}, separator='para'), Run(text='dreizeilige', font='Vollkorn Black Italic', fontsize=30, fcolor='Gelb'), Run(text='', has_itext=False, paragraph_style='idml/normalparagraphstyle', paragraph_attrs={'ALIGN': '0', 'LINESPMode': '1'}, separator='para'), Run(text='Headline', font='Gotham Narrow Ultra', fontsize=30, fcolor='White', paragraph_style='idml/normalparagraphstyle')],
        trail_attrs={'LINESPMode': '1'},
    ))
    # h_mm widened 8.7340mm→12.6389mm: Scribus clips lines when frame_h < 2 explicit lines × line height (leading=14.22pt; IDML overflows silently)
    # P5/playbook y_mm_shift.py: y_mm 89.966 → 90.466 (calibration probe (+0.5mm))
    # P5/playbook y_mm_shift.py: y_mm 90.466 → 88.6033 (uniform ++5.28pt × sign=-1 → -1.8627mm)
    page0.add(TextFrame(
        x_mm=78.5263,
        y_mm=88.6033,
        w_mm=65.13,
        ...
        layer=0,
        style='idml/normalparagraphstyle',
        runs=[Run(text='Mehrzeilige Subheadline –', font='Gotham Narrow Book', fontsize=13, fcolor='White', paragraph_style='idml/normalparagraphstyle', paragraph_attrs={'ALIGN': '0', 'LINESPMode': '1'}), Run(text='', has_itext=False, paragraph_style='idml/normalparagraphstyle', paragraph_attrs={'ALIGN': '0', 'LINESPMode': '1'}, separator='para'), Run(text='mehr Info zum Thema', font='Gotham Narrow Book', fontsize=13, fcolor='White', paragraph_style='idml/normalparagraphstyle')],
        trail_attrs={'LINESPMode': '1'},
    ))
    page0.add(Polygon(
        x_mm=161,
...
def _add_page_4(doc: Document, page4) -> None:  # overrides task-3 stub
    ...
    # noinject: green-pine-trees-covered-with-fog.jpg is a CMYK JPEG; Scribus
    # 1.6.x renders CMYK JPEGs blank (authoring-bug — see TOLERANCE_LOG.md).
    page4.add(ImageFrame(
        ...
        anname='u906',
        layer=0,
        image='../../shared/assets/26-03-flyer-a6-querformat-zweigeteilt/green-pine-trees-covered-with-fog.jpg',
...
def _add_page_5(doc: Document, page5) -> None:  # overrides task-3 stub
    ...
    # noinject: plakat-dunkel-fuer-flyer.png is the design's intended back-cover
    # poster artwork; no library substitute applies (renders correctly).
    page5.add(ImageFrame(
        ...
        layer=0,
        rotation_deg=-90,
        image='../../shared/assets/26-03-flyer-a6-querformat-zweigeteilt/plakat-dunkel-fuer-flyer.png',
        local_scale=(0.168027, 0.168027),
        scale_type=1,
    ))
    # h_mm widened 19.5185mm→38.4528mm: Scribus clips lines when frame_h < 5 explicit lines × line height (leading=14.00pt; IDML overflows silently)
    page5.add(TextFrame(
        x_mm=30.38,
        y_mm=46.2911,
        w_mm=87.24,
        ...
        layer=0,
        style='idml/normalparagraphstyle',
        runs=[Run(text='Ich bin längeres Zitat. Ebis pror sanit voluptiae doloria menduci istiur se plam corempor ad que il. Onsequas eaquame quae doloressitia cullace stius, imilia.', font='Vollkorn Bold Italic', fontsize=14, fcolor='White', paragraph_style='idml/normalparagraphstyle', paragraph_attrs={'ALIGN': '1', 'LINESPMode': '1'})],
        trail_attrs={'ALIGN': '1', 'LINESPMode': '1'},
    ))
    ...
    # tune: ub52 y_mm reverted to scaffold value 69.4096 — the y_mm_shift
    # playbook oscillated it (alternating ±2.4/±4.8pt) without converging.
    # The audit's 'uniform offset' is a line-count-mismatch artifact
    # (cross-renderer wrap), not a real anchor offset. See REVIEW_NOTES.md.
    page5.add(TextFrame(
        x_mm=53.1686,
        y_mm=69.4096,
        w_mm=41.6629,
