...
def _add_page_0(doc: Document, page0) -> None:  # overrides task-3 stub
    ...
    # noinject: u9be — CMYK JPEG (green-pine-trees-covered-with-fog.jpg) renders blank/faint in Scribus 1.6.x — links_export.py passthrough keeps CMYK; authoring-bug, residual accepted
    page0.add(ImageFrame(
        ...
        anname='u9be',
        layer=0,
        image='../../shared/assets/26-03-flyer-a6-querformat-quadrat-in-bild/green-pine-trees-covered-with-fog.jpg',
        local_scale=(0.756942, 0.756942),
        scale_type=1,
        local_offset_mm=(-6.25, 0),
    ))
    page0.add(Polygon(
        x_mm=42.5,
        y_mm=21,
    ...
    page0.add(ImageFrame(
        ...
        anname='ua5a',
        layer=0,
        # ua5a: white Grünen logo — switched inline PNG → image= ref + scale_type=0;
        # Scribus 1.6.x renders SCALETYPE=1 + small frame + RGBA white PNG transparent.
        image='../../shared/assets/26-03-flyer-a6-querformat-quadrat-in-bild/gruene-logo-bund-weiss-cmyk.png',
        scale_type=0,
    ))
...
def _add_page_4(doc: Document, page4) -> None:  # overrides task-3 stub
    ...
    # noinject: u906 — CMYK JPEG (green-pine-trees-covered-with-fog.jpg) renders blank in Scribus 1.6.x — links_export.py passthrough keeps CMYK; authoring-bug, residual accepted
...
def _add_page_5(doc: Document, page5) -> None:  # overrides task-3 stub
    ...
    # noinject: ua88 — CMYK JPEG (leonore-sitzend-kopie.jpg) discolours in Scribus 1.6.x — links_export.py passthrough keeps CMYK; authoring-bug, residual accepted
    page5.add(ImageFrame(
        ...
        anname='ua88',
        layer=0,
        image='../../shared/assets/26-03-flyer-a6-querformat-quadrat-in-bild/leonore-sitzend-kopie.jpg',
        local_scale=(0.555048, 0.555048),
        scale_type=1,
        local_offset_mm=(-4.4642, 0),
    ))
    # noinject: ua8f — CMYK PSD (schwarzer-verlauf-radial.psd) via convert -flatten produces non-ICC CMYK->RGB output that posterizes/discolours; authoring-bug, residual accepted
    page5.add(ImageFrame(
        x_mm=22,
        y_mm=3,
        w_mm=135,
    ...
    # P5/playbook y_mm_shift.py: y_mm 71.1685 → 71.6685 (calibration probe (+0.5mm))
    page5.add(TextFrame(
        x_mm=75.8352,
        y_mm=71.6685,
        w_mm=41.6629,
