"""Smoke test: v2 dunkelgrun-rules-between-items-v2 passes envelope (T13)."""
...
def _load_variant():
    spec = importlib.util.spec_from_file_location(
        "_v2_dunkelgrun_rules_between_items",
        ROOT / "experiments" / "falzflyer-p2-mein-plan-v2"
        / "variants" / "dunkelgrun-rules-between-items-v2.py",
    )
    assert spec is not None and spec.loader is not None
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
...
class DunkelgrunRulesV2Test(unittest.TestCase):
    ...
    def test_builds_and_passes_envelope(self):
        ...
        self.assertEqual(
            errors, [],
            f"dunkelgrun-rules-between-items-v2 must pass envelope; got: "
